...
public class ObjSpawnManager : MonoBehaviour {
    ...
    IEnumerator WaveSpawner()
    {
        ...
        while (true)
        {
            GameObject[] allObjs = GameObject.FindGameObjectsWithTag(tag);//todo increase performance
...
//        Debug.Log(worldSpawnPoint);
        RaycastHit hit;
        if (Physics.Raycast(worldSpawnPoint, Vector3.down, out hit, 200,layerMask))
        {
