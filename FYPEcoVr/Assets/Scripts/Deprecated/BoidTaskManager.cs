...
public class BoidTaskManager : MonoBehaviour
{
    ...
    public AnBoidMove boidMovementScript;
    ...
    public void Initialize()
    {
        ...
        {
            print("error making animal task manager, no profile");
        }
    }
    ...
    public void CreateObjs()
    {
        ...
        if (sensorySphere == null)
        {
            ...
            BoidSensoryScript sensescript = sensorySphere.AddComponent<BoidSensoryScript>();
...
//            Debug.Log("numOfTasks"+numOfTasks);
        ...
        else//move toward location//todo move this to new script
        {
            ...
            if (Vector3.Distance(this.transform.position, target.transform.position) > targetDistToTarget)//If over a certain distance then keep moving towards it
            {
                boidMovementScript.targetTransform = target.transform;
            }
